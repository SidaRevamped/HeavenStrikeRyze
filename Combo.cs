        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            if (Program.mode == "AoE/Shield" && Player.Level != 1)
            {
                ...
                MainTarget = tarqs.Where(x => Helper.HasEBuff(x) && Program._q.GetPrediction(x).Hitchance >= HitChance.Low && Program._q.IsReady()
                    && Helper.GetchainedTarget(x).Count() >= 2 && Helper.GetchainedTarget(x).Count(y => y is Obj_AI_Hero) >= 1)
                    .MaxOrDefault(x => Helper.GetchainedTarget(x).Count(y => y is Obj_AI_Hero));
                if (MainTarget != null)
                {
                ...
                MainTarget = tars.Where(x => Helper.HasEBuff(x) && Program._e.IsReady()
                    && Helper.GetchainedTarget(x).Count(y => y is Obj_AI_Hero) >= 1)
                    .MaxOrDefault(x => Helper.GetchainedTarget(x).Count(y => y is Obj_AI_Hero));
                if (MainTarget != null)
                {
